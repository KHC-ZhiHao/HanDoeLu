<template>
    <div id="driver">
        ...
        <div v-else class="unit" v-for="layout in config.layout" :key="layout + random()" @click="press">
            <div class="button setting" v-if="layout === 4095" @click="toSetting()"></div>
            <div
                class="button"
                v-else-if="config.button[layout]"
        ...
        mounted () {
            ...
            if (window.DeviceOrientationEvent) {
                ...
                this.interval = setInterval(() => {
                    this.socket.emit('control', control)
                }, 1)
            } else {
                alert('This browser not support 6 axis.') 
            }
        },
        methods: {
            toSetting() {
